(uiop:define-package :moo (:use :cl))
(in-package :moo)
...
(defmethod print-object ((obj ltype) stream)
  (format stream "#<NAKED-TYPE ~a>" (type-of obj)))

(defmethod print-object ((obj type-ref) stream)
  (print-type (deref obj) stream))

(defmethod print-type ((obj ltype) stream)
  (format stream "#T(~a)" (slot-value obj 'name)))
...
(defun init-type (name &rest args)
  nil)

;;------------------------------------------------------------

(defgeneric unify (type-a type-b))
...
(defun infer (context expression)
  ...
  (flet ((%infer ()
           (cond
             ...
             (t
              (infer-literal context expression)))))
    `(the ,(init-type (%infer)) ,expression)))
